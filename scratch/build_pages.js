...
const cventPath = path.join('e:', 'Cvent', 'Cvent.html');
const outDir = path.join('e:', 'Cvent', 'event-types');

// Read reference
const cventHtml = fs.readFileSync(cventPath, 'utf8');

// Regex matches
const headMatch = cventHtml.match(/(<!DOCTYPE html>[\s\S]*?<head>[\s\S]*?<\/head>)/i);
const headerMatch = cventHtml.match(/(<header id="main-header"[\s\S]*?<\/header>)/i);
const footerMatch = cventHtml.match(/(<footer[\s\S]*?<\/footer>)/i);

if (!headMatch || !headerMatch || !footerMatch) {
    console.error("Could not extract parts");
    process.exit(1);
}

const headHtml = headMatch[1].replace(/\.\//g, '../');
const headerHtml = headerMatch[1].replace(/\.\//g, '../');
const footerHtml = footerMatch[1].replace(/\.\//g, '../');

if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
}

// Read Python file content we just generated to get the bodies, or we can just embed them here.
// Actually, it's safer to embed the body directly.
const corporateTravelBody = `
...
<body class="antialiased">
        ...
        <div class="max-w-[1200px] mx-auto flex flex-col lg:flex-row items-center gap-12 relative z-10">
            ...
            <div class="lg:w-1/2 text-left">
                <div class="text-[13px] font-bold tracking-widest text-gray-500 uppercase mb-4 flex items-center">
                    <a href="#" class="hover:text-[#006ae1] mr-2">CVENT PLATFORM</a> <span class="mx-2">&gt;</span> <span class="text-[#0d1240] ml-2">FLAGSHIP EVENTS</span>
                </div>
    ...
    <section class="py-20 bg-white text-center">
        ...
        <div class="max-w-[1000px] mx-auto flex flex-wrap justify-center items-center gap-12 opacity-60 grayscale">
            <div class="font-bold text-2xl text-gray-400">PENNSTATE</div>
            <div class="font-bold text-2xl text-gray-400">Morgan Stanley</div>
            <div class="font-bold text-2xl text-gray-400">Wrike</div>
            <div class="font-bold text-2xl text-gray-400">Zoom</div>
            <div class="font-bold text-2xl text-gray-400">okta</div>
        </div>
    </section>
    ...
    <section class="py-24 bg-[#f8fafc]">
        <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
                <div class="flex flex-col items-center">
                    <div class="text-[#006ae1] font-bold text-[20px] mb-2 border-b-2 border-[#006ae1] pb-2 px-4 cursor-pointer">Cvent Event Management</div>
                </div>
                <div class="flex flex-col items-center">
                    <div class="text-[#0d1240] font-bold text-[20px] mb-2 border-b-2 border-transparent hover:border-gray-300 pb-2 px-4 cursor-pointer">Attendee Hub</div>
                </div>
                <div class="flex flex-col items-center">
                    <div class="text-[#0d1240] font-bold text-[20px] mb-2 border-b-2 border-transparent hover:border-gray-300 pb-2 px-4 cursor-pointer">OnArrival</div>
                </div>
            </div>

            <!-- Cards Grid -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left max-w-[900px] mx-auto">
                <div class="bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 p-8 hover:shadow-md transition-shadow">
    ...
    <!-- Bottom CTA -->
    <section class="py-20 text-center text-white" style="background: linear-gradient(135deg, #2462e8 0%, #7c3aed 100%);">
...
fs.writeFileSync(path.join(outDir, 'corporate-travel.html'), headHtml + corporateTravelBody + "</html>");
fs.writeFileSync(path.join(outDir, 'complex-events.html'), headHtml + complexEventsBody + "</html>");
